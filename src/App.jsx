...
function MindfulMoment(){ const idx=(new Date().getDate()+new Date().getMonth())%QUOTES.length; const [showBreath,setShowBreath]=useState(false); return (<Card className="rounded-2xl bg-gradient-to-r from-indigo-500/10 via-sky-500/10 to-emerald-500/10"><CardContent className="p-5 space-y-3"><div className="flex items-center justify-between gap-2"><div><p className="text-xs text-muted-foreground">Mindful Moment</p><p className="text-base sm:text-lg mt-1">“{QUOTES[idx]}”</p></div><Button size="sm" className="whitespace-nowrap" onClick={()=> setShowBreath(v=>!v)}>Take a Breath</Button></div>{showBreath && <BreathCoach onClose={()=> setShowBreath(false)} />}</CardContent></Card>); }
    ...
    <Card className={`rounded-2xl ${glass? 'supports-[backdrop-filter]:backdrop-blur bg-white/40 dark:bg-slate-900/30 border-white/40 dark:border-white/10':''}`}>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          {habits.map(h=> (
            <div key={h.id} className="relative border rounded-2xl p-3">
  ...
  return (
    <AmbientProvider>
      ...
      <div className="min-h-[100dvh] bg-gradient-to-b from-background to-background/80">
        <div className="max-w-7xl mx-auto p-4 sm:p-6 pb-[env(safe-area-inset-bottom)] pb-6">
